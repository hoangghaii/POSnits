...
@Component({
  selector: 'app-staff-shift-calendar',
  templateUrl: 'staff-shift-calendar.component.html',
  styleUrls: ['staff-shift-calendar.component.scss']
})
export class StaffShiftCalendarComponent implements OnInit, OnChanges {
  localTime = LocalTime;
  ...
  refreshCalendar(): void {
    ...
    this.choiceDate.year = moment(this.days[0]).format('YYYY');
    this.choiceDate.month = moment(this.days[0]).format('MM');
    this.choiceDate.date = moment(this.days[0]).format('DD');
    this.renderCalendar(this.choiceDate.year, this.choiceDate.month, this.choiceDate.date);
  }
    ...
    for (let row = 0; row < dateOfMoth.length / 7; row++) {
      ...
      for (let col = 0; col < 7; col++) {
        ...
        if (this.listStaffShift && this.listStaffShift.length) {
          // Create span
          const spanEle: HTMLTableColElement = this.renderer.createElement('span');

          if (cssClass.includes('dis')) {
            spanEle.innerText = ''; // Set html
          } else {
            if (this.listStaffShift[item?.date]) {
              spanEle.innerText = '所属店'; // Set html
              spanEle.setAttribute('class', 'non-empty'); // Set class
            }
          }

          tdEle.insertAdjacentElement('beforeend', spanEle);
        }
  ...
  handleSelectDay(): void {
    if (!this.multi && this.days.length > 0) {
      return this.selectDay.emit(this.days[this.days.length - 1]);
    }
    this.selectDay.emit(this.days);
  }
}
